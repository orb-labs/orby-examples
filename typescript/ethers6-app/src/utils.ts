import { Account, AccountCluster, Activity, ActivityStatus, OnchainOperation } from "@orb-labs/orby-core";
...
export async function setup(
  ...
  const virtualNodeRpcUrl = await privateInstanceProvider.getVirtualNodeRpcUrl(accountClusterId, chainId, address);
...
export const onOperationSetStatusUpdateCallback = async (activity?: Activity): Promise<void> => {
  console.log("[INFO] [onOperationSetStatusUpdateCallback] response: ", activity);

  if (!activity) {
    // handle not found
  } else if ([ActivityStatus.SUCCESSFUL, ActivityStatus.PENDING].includes(activity?.overallStatus)) {
    // handle success
  } else if ([ActivityStatus.FAILED, ActivityStatus.NOT_FOUND].includes(activity?.overallStatus)) {
    // handle other cases
  }
};
